...
class RaboDevices:
    def __init__(self, config: CollectorConfig, mode: str = "sim") -> None:
        ...
        rabo = config.rabo
        self._arm_limits = self._joint_limit_spec(LinkerArmA7, 7)
        self._left_hand_limits, self._left_hand_drive = self._hand_spec(LinkerHandO6Left)
        self._right_hand_limits, self._right_hand_drive = self._hand_spec(LinkerHandO6Right)
        ...
        self._command_executor = concurrent.futures.ThreadPoolExecutor(max_workers=4, thread_name_prefix="rabo-command")
    ...
    def read_full_state(self) -> np.ndarray | None:
        ...
        if any(not self._valid_part(part, size) for part, size in zip(parts, expected)):
            return None
        values = np.asarray([float(value) for part in parts for value in part], dtype=np.float32)
    ...
    @staticmethod
    def _expand_hand_target(active_target: np.ndarray, current_full: np.ndarray, limits: np.ndarray, drive: np.ndarray, max_step: float) -> np.ndarray:
        ...
        fraction = np.divide(active_target - active_limits[:, 0], span, out=np.zeros_like(active_target), where=span > 0)
    ...
    def command_action(self, action: np.ndarray, current_full_state: np.ndarray, scope: str, max_arm_step: float, max_hand_step: float) -> dict[str, bool]:
        ...
        left_arm = np.clip(self._slew(action[0:7], current[0:7], max_arm_step), self._arm_limits[:, 0], self._arm_limits[:, 1])
        right_arm = np.clip(self._slew(action[7:14], current[7:14], max_arm_step), self._arm_limits[:, 0], self._arm_limits[:, 1])
        commands: list[tuple[str, object, np.ndarray]] = [
            ("left_arm", self.left_arm, left_arm),
            ("right_arm", self.right_arm, right_arm),
        ]
        if scope == "full":
            left_hand = self._expand_hand_target(action[14:20], current[14:25], self._left_hand_limits, self._left_hand_drive, max_hand_step)
            right_hand = self._expand_hand_target(action[20:26], current[25:36], self._right_hand_limits, self._right_hand_drive, max_hand_step)
            commands.extend((("left_hand", self.left_hand, left_hand), ("right_hand", self.right_hand, right_hand)))

        futures = {
            name: self._command_executor.submit(device.move_joints, target.tolist(), blocking=False)
            for name, device, target in commands
        }
        results = {name: bool(future.result(timeout=2.0)) for name, future in futures.items()}
    ...
    def pre_position(self) -> None:
        ...
        def right() -> None:
            self._move_joints_checked(self.right_arm, "right pre-position 1", [-1.57, -1.5, 0, -1.57, 0, -1, 0])
            self._move_joints_checked(self.right_arm, "right pre-position 2", [0, 0, 0, -2, 0, 1, 0])

        def left() -> None:
            self._move_joints_checked(self.left_arm, "left pre-position 1", [0, -1.57, 0, 0, 0, 0, 0])
            self._move_joints_checked(self.left_arm, "left pre-position 2", [-1.57, -0.7, 0, 0, 0, 0, 0])

        threads = [threading.Thread(target=guarded, args=(right,)), threading.Thread(target=guarded, args=(left,))]
